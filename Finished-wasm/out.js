...
function wasm_display_surface(width, height, format, stride, data) {
...
function wasm_display_update(data, stride, x, y, width, height) {
 ...
 if (surface) {
  ...
  surface.context.putImageData(surface.image, 0, 0, x, y, width, height);
 }
...
function ___pthread_create_js(pthread_ptr, attr, startRoutine, arg) {
 ...
 var transferList = [];
 var error = 0;
 var transferredCanvasNames = attr ? HEAPU32[(((attr) + (40)) >>> 2) >>> 0] : 0;
   ...
   if (GL.offscreenCanvases[name]) {
    ...
    if (canvas.transferControlToOffscreen) {
     if (!canvas.canvasSharedPtr) {
      ...
      HEAP32[((canvas.canvasSharedPtr) >>> 2) >>> 0] = canvas.width;
      HEAP32[(((canvas.canvasSharedPtr) + (4)) >>> 2) >>> 0] = canvas.height;
      HEAPU32[(((canvas.canvasSharedPtr) + (8)) >>> 2) >>> 0] = 0;
     }
     offscreenCanvasInfo = {
 ...
 for (var canvas of Object.values(offscreenCanvases)) {
  HEAP32[(((canvas.canvasSharedPtr) + (8)) >>> 2) >>> 0] = pthread_ptr;
 }
 var threadParams = {
...
var wasmImports = {
 ...
 /** @export */ wasm_display_surface: wasm_display_surface,
 /** @export */ wasm_display_update: wasm_display_update
};
...
var ___start_em_js = Module["___start_em_js"] = 7863880;

var ___stop_em_js = Module["___stop_em_js"] = 7877937;
